...
def hitungBobot(listsolusi,k):
    # Urutkan bobot berdasarkan listsolusi
    # Idenya cari yang terdekat ke 24, itu yang bobotnya terendah
    listbobot = [0,0,0,0]
    n = 1
    idxmax = 0
    for i in range(1,4):
    ...
    for i in range(0,4):
        ...
        for j in range(0,4):
            if (abs(24-listsolusi[j])<=abs(24-listsolusi[idxmin])) and (not listbobot[j]):
                if (abs(24-listsolusi[j])==abs(24-listsolusi[idxmin])):
                    sama = True
                else:
                    sama = False
        ...
        if (minval == 0):
            ...
            listbobot[idxmin] = n+k*10+abs(24-listsolusi[idxmin])
            if not sama:
                n+=1
    return listbobot
